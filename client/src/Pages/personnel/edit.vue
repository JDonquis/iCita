<script>
export default { name: 'personnelEdit' }
</script>
...
const photoUrl = (path) => {
  if (!path || path === 'nophoto.webp') return null;
...
onMounted(async () => {
  ...
  try {
    const { data } = await axios.get(`/api/personnel/${id}`);
    ...
    if (data.person.photo && data.person.photo !== 'nophoto.webp') {
      photoPreview.value = photoUrl(data.person.photo);
    }
  } catch(e) {
    alert('Error al cargar datos del personal:',e);
...
<template>
<AppLayout titulo="Personal Médico" subtitulo="Editar registro de personal">
  <div class="p-4 md:p-8 flex-1 overflow-y-auto">
    ...
    <header class="flex justify-between items-center mb-6 md:mb-8">
      <div class="flex items-center gap-2 md:gap-4">
        ...
        <h2 class="text-headline-lg-mobile md:text-headline-lg font-headline-lg-mobile md:font-headline-lg text-primary tracking-tight">Editar Personal</h2>
      </div>
    ...
    <div v-if="fetchLoading" class="flex justify-center p-12">
      <span class="material-symbols-outlined animate-spin text-primary text-4xl">sync</span>
    </div>

    <div v-else class="glass-panel rounded-2xl md:rounded-[32px] overflow-hidden flex flex-col p-6 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
